...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-17 * int((ss-1)->staticEval + ss->staticEval), -1752, 1833);
    ...
    // return a fail low.
    if (eval < alpha - 471 - 250 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        &&  depth < 9
        &&  eval - futility_margin(depth, improving) - (ss-1)->statScore / 274 >= beta
        &&  eval >= beta
        &&  eval < 25343) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;
