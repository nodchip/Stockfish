...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1257 - int(delta) * 1025 / int(rootDelta)) / 918 + (!i && r > 931);
  }
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        ...
        && (ss-1)->statScore < 18006
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 22 * depth + 237
        && !excludedMove
        ...
        &&  beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 165, 6) + depth / 3 + 4;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (bestValue + 59 + 12 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 523 && ss->doubleExtensions <= 9;
      ...
      if (move != bestMove)
      {
          if (capture && captureCount < 24)
              capturesSearched[captureCount++] = move;

          else if (!capture && quietCount < 86)
